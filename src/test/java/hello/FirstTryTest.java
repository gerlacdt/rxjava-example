...
public class FirstTryTest {
	...
	public void observableNonBlocking() {
		Observable<String> customObservableNonBlocking = customObservableNonBlocking();
		
		customObservableNonBlocking.subscribe((it) -> {
			System.out.println(it);
		});
	}
	
	private Observable<String> customObservableBlocking() {
		Observable<String> o = Observable.create(aSubscriber -> {
	...
	public void collectTest() {
		...
		Observable.range(1, 10).subscribeOn(Schedulers.io())						     
				  .subscribe(i -> System.out.println("i = " + randInt()));
